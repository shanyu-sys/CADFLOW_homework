...
class Component(object):
    ...
    def matrixFill(self, G, B, vectorI, vectorE):
        ...
        if self.name.startswith('Z'):  # Z handling
            if isnotGround(self.pNode) and isnotGround(self.nNode):
                G[nP][nP] += 1 / value
                G[nN][nN] += 1 / value
                G[nP][nN] -= 1 / value
...
with open(netlistFile) as netlist:
    ...
    vectorI = np.zeros([N, 1], dtype=complex)
    vectorE = np.zeros([M, 1], dtype=complex)
    ...
    for node in nodeDict.keys():
        print("node %s: %sV" % (node, Result[nodeDict[node]]))
    if usingMNA:
     for vsrc in vsrcDict.keys():
            print("vsource %s: %sA" % (vsrc, Result[vsrcDict[vsrc] + N]))
    # sort by name. The node numbers should be sorted as int
    #nodeList1 = ['Z9' if x == 'PIN' else x for x in nodeList]
    #nodeList1.sort(key=lambda x: (x[0], int(x[1:])))
    #for node in nodeList1:
    #   if node == 'Z9':
    #        print("node %s: %.6fV" % ('PIN', Result[nodeDict['PIN']]))
    #    else:
    #        print("node %s: %.6fV" % (node, Result[nodeDict[node]]))
    #if usingMNA:
    #    for vsrc in vsrcDict.keys():
    #        print("vsource %s: %.6fA" % (vsrc, Result[vsrcDict[vsrc] + N]))
    # sort by the value of voltage and circuit
    #nodeList.sort(key=lambda x: abs(Result[nodeDict[x]]))
    #for node in nodeList:
    #    print("node %s: %.6fV" % (node, Result[nodeDict[node]]))

    #vsrcList.sort(key=lambda x: Result[vsrcDict[x]])
    #if usingMNA:
     #   for vsrc in vsrcList:
     #       print("vsource %s: %.6fA" % (vsrc, Result[vsrcDict[vsrc] + N]))
