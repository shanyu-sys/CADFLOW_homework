...
def ladderWave(N=10, endTime=1000, deltaTime=0.01, tpdMode=False):
    ...
    The uniform ladder has a 1 Ohm resistor and a 1 Farady capacitor
    ...
    A = np.eye(N) + deltaTime * np.dot(np.linalg.inv(C), G)
...
def propagationDelay(v, t, n):
    for i in range(len(t)):
        if v[i] > 0.5:
            print("Propagation delay of N=%d is %.2f" % (n, t[i]))
            return t[i]
...
if __name__ == '__main__':
    ...
    propagationDelay(v, t, 10)

    v, t = ladderWave(N=5)
    plot_wave(t, v, 'c', "N=5")
    propagationDelay(v, t, 5)

    v, t = ladderWave(N=20)
    plot_wave(t, v, 'g', "N=20")
    propagationDelay(v, t, 20)

    v, t = ladderWave(N=50)
    plot_wave(t, v, 'y', "N=50")
    propagationDelay(v, t, 50)

    vol = 0.1 * np.ones([len(t), 1])
    plot_wave(t, vol, ':')
    vol = 0.5 * np.ones([len(t), 1])
    plot_wave(t, vol, '--')
    vol = 0.9 * np.ones([len(t), 1])
    plot_wave(t, vol, '-.')
