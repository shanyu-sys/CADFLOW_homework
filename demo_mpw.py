...
def not_fully_inside(x, y, w, h, radius):
    R2 = radius * radius
    if sdist(x, y) > R2 or sdist(x + w - 1, y) > R2 \
    or sdist(x, y + h - 1) > R2 \
    or sdist(x + w - 1, y + h - 1) > R2:
        return True
    else:
        return False

def next_not_fully_inside(x, y, w, h, radius):
    boxes = [
        (x + w, y, w, h),
        (x + w, y + h, w, h),
        (x, y + h, w, h),
    ...
    for (xx, yy, ww, hh) in boxes:
        if not_fully_inside(xx, yy, ww, hh, radius):
            count = count + 1
    if count > 0:
        return True
    else:
...
def mpw(l):
    ...
    wrap is the dimensions of boundary box of all planned dice.
    '''
    #planned = [(0, 0, 15, 15), \
     #    (15, 0, 15, 10), \
     #    (15, 10, 8, 8), \
    ...
    # the way to automatically generate wrap
    max_x = 0
    max_y = 0

    for (x, y, x_w, y_h) in planned:
        if x + x_w > max_x:
            max_x = x + x_w
        if y + y_h > max_y:
            max_y = y + y_h
    ...
    print("The width and height of the field is"),
    print(wrap)
...
def field(width=30, height=30, radius=300, det=False):
    ...
    for (x, y) in xy_list:
        if not fully_outside(x, y, width, height, radius):
            if not_fully_inside(x, y, width, height, radius):
                cv.create_rectangle(x_c + x, y_c + y, x_c + x + width, y_c + y + height, width=1, outline='blue')
            elif next_not_fully_inside(x, y, width, height, radius):
                cv.create_rectangle(x_c + x, y_c + y, x_c + x + width, y_c + y + height, width=1, outline='green')
            else:
                cv.create_rectangle(x_c + x, y_c + y, x_c + x + width, y_c + y + height, width=1)
            ...
            if det:
                for (xx, yy, ww, hh) in dice_list:
                    ...
                    y1 = y_c + y + yy
                    if not_fully_inside(x, y, width, height, radius):
                        cv.create_rectangle(x1, y1, x1 + ww, y1 + hh, outline='blue')
                    elif next_not_fully_inside(x, y, width, height, radius):
                        cv.create_rectangle(x1, y1, x1 + ww, y1 + hh, outline='green')
                    else:
